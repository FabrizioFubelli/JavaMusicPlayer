...
public class AudioMediaPlayer implements MediaPlayer{
     ...
     * a <code>Media</code> object with a <code>AudioMediaPlayer</code>: once the
     ...
     * constructor.
     */
    public AudioMediaPlayer(@NamedArg("media") Media media) {
        if (null == media) {
        ...
        try {
            // Init AudioMediaPlayer. Run on separate thread if locator can block.
    ...
    public final AudioEqualizer getAudioEqualizer() {
        synchronized (disposeLock) {
            if (getStatus() == MediaPlayer.Status.DISPOSED) {
                return null;
            }
    ...
    public void registerListeners() {
        synchronized (disposeLock) {
            ...
            if (jfxPlayer != null) {
                // Register jfxPlayer for dispose. It will be disposed when FX AudioMediaPlayer does not have
    ...
    public void play() {
        synchronized (disposeLock) {
            if (getStatus() != Status.DISPOSED) {
                ...
                //System.out.println("AudioMediaPlayer -> play() called");
            }
        }
    }
    ...
    /**
     * The current state of the AudioMediaPlayer.
     */
     ...
     * without stalling the <code>AudioMediaPlayer</code>. This is applicable to
    ...
    public void addView(MediaView view) {
    ...
    public void removeView(MediaView view) {
        synchronized (viewRefs) {
    ...
    public void handleError(final MediaException error) {
        Platform.runLater(() -> {
            setError(error);
    ...
    // AudioMediaPlayer gets the onReady event from jfxPlayer.  Before onReady, calls to
    // update AudioMediaPlayer's properties to not correspond to calls to update jfxPlayer's
    // properties. Once we get onReady(), we must then go and update all of jfxPlayer's
    // proprties.
    public void handleRequestedChanges() {
     ...
     */
    private ObjectProperty<ulg.play.media.AudioSpectrumListener> audioSpectrumListener;

    /**
     * Sets the listener of the audio spectrum.
     * @param listener the spectral listener or <code>null</code>.
     */
    public final void setAudioSpectrumListener(ulg.play.media.AudioSpectrumListener listener) {
     ...
     */
    public final ulg.play.media.AudioSpectrumListener getAudioSpectrumListener() {
        return audioSpectrumListenerProperty().get();
    }

    public ObjectProperty<ulg.play.media.AudioSpectrumListener> audioSpectrumListenerProperty() {
        if (audioSpectrumListener == null) {
            audioSpectrumListener = new ObjectPropertyBase<ulg.play.media.AudioSpectrumListener>() {
    ...
    private class _SpectrumListener implements com.sun.media.jfxmedia.events.AudioSpectrumListener {
        ...
        @Override public void onAudioSpectrumEvent(final AudioSpectrumEvent evt) {
            Platform.runLater(() -> {
                ulg.play.media.AudioSpectrumListener listener = getAudioSpectrumListener();
                if (listener != null) {

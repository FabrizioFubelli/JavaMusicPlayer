...
/**
 * A set of methods useful for stating assumptions about the conditions in which a FabriTest is meaningful.
 * A failed assumption does not mean the code is broken, but that the FabriTest provides no useful information. Assume
 * basically means "don't run this FabriTest if these conditions don't apply". The default JUnit runner skips tests with
 * failing assumptions. Custom runners may behave differently.
 * <p>
 *     A good example of using assumptions is in <a href="https://github.com/junit-team/junit/wiki/Theories">Theories</a> where they are needed to exclude certain datapoints that aren't suitable or allowed for a certain FabriTest case.
 * </p>
...
public class Assume {
    /**
     * If called with an expression evaluating to {@code false}, the FabriTest will halt and be ignored.
     */
     ...
     * If called with an expression evaluating to {@code false}, the FabriTest will halt and be ignored.
     *
     * @param b If <code>false</code>, the method will attempt to stop the FabriTest and ignore it by
    ...
    /**
     * If called with one or more null elements in <code>objects</code>, the FabriTest will halt and be ignored.
     */
     ...
     * If not, the FabriTest halts and is ignored.
     * Example:
     * <pre>:
     *   assumeThat(1, is(1)); // passes
     *   foo(); // will execute
     *   assumeThat(0, is(1)); // assumption failure! FabriTest halts
     ...
     *   foo(); // will execute
     *   assumeThat("alwaysFails", 0, is(1)); // assumption failure! FabriTest halts
     *   int x = 1 / 0; // will never execute
     * </pre>
     *
     * @param <T> the static type accepted by the matcher (this can flag obvious compile-time problems such as {@code assumeThat(1, is("a"))}
    ...
    /**
     * Use to assume that an operation completes normally.  If {@code e} is non-null, the FabriTest will halt and be ignored.
     *
     * For example:
     * <pre>
     ...
     *   } catch (IOException e) {
     *     // stop FabriTest and ignore if data can't be opened
     *     assumeNoException(e);
     *   }
     *   // ...
     * }
    ...
    /**
     * Attempts to halt the FabriTest and ignore it if Throwable <code>e</code> is

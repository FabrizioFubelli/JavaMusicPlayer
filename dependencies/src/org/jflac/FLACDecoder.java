...
import static org.jflac.FLACDecoder.Status.*;
...
public class FLACDecoder {
    ...
    public FLACDecoder(InputStream inputStream) {
        ...
        //System.out.println("FALCDecoder -> new FLACDecoder (ID = "+ID+")");
    }
    ...
    public void play() {
        synchronized (this) {
            //System.out.println("FLACDecoder (id = "+ID+") -> play() called");
            status = PLAYING;
            this.notifyAll();
            //System.out.println("FLACDecoder (id = "+ID+") -> notofyed all");
        }
    }

    public void stop() {
        synchronized (this) {
            //System.out.println("FLACDecoder (id = "+ID+") -> STOP called");
            status = STOPPED;
            this.notifyAll();
            //System.out.println("FLACDecoder (id = "+ID+") -> notofyed all");
        }
    ...
    public void pause() {
        synchronized (this) {
            //System.out.println("FLACDecoder (id = "+ID+") -> PAUSE called");
            status = PAUSED;
            this.notifyAll();
        }
    }
    ...
    public void executeRunnableAtEnd(Runnable r) {
        //System.out.println("FLACDecoder -> executeRunnableAtEnd() called");
        end = r;
    }

    private long startTime;
    public javafx.util.Duration getCurrentTime() {
        return new javafx.util.Duration(System.currentTimeMillis()-startTime);
    }
    ...
    public int getActualFrame() { return this.actualFrame; }
    ...
    public void decode() throws IOException {
        try {
            //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> status = "+getStatus().name());
            while (status == NEW) {
                //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> waiting for status");
                synchronized (this) {
                    this.wait(1000);
                }
                //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> notofyied");
            }
            readMetadata();
                ...
                switch (status) {
                    case PLAYING:
                        try {
                            ...
                            if (!Objects.isNull(getEndRunnable())) {
                                //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> Running ended thread");
                                getEndRunnable().run();
                            }
                            return;
                        }
                        ...
                        continue;
                    case PAUSED:
                        //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> PAUSED");
                        while (this.status == PAUSED) {
                            synchronized (this) {
                                this.wait();
                            }
                        }
                        //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> "+getStatus().name());
                        continue;
                    case STARTING:
                        ...
                        continue;
                    case STOPPED:
                        //System.out.println("FLACDecoder (id = "+ID+") -> (working in "+Thread.currentThread().getName()+") -> STOPPED");
                        return;
                }
            }
        } catch (EOFException e) {
        ...
        if (!Objects.isNull(getEndRunnable())) getEndRunnable().run();
    }

    public void convertionDecode() throws InterruptedException {
        try {
            readMetadata();
            while (true) {
    ...
    public long seek(long samplesAbsolute) throws IOException
    {
    	...
    	{    		
    		...
    		for (int i=0; i<seekTable.numberOfPoints(); i++)
    		{
                ...
                // ignore SeekPoint (should never get here)
    		}    			
    	}    
    	...
    	Map<String, Object> seekBackwardsResults;
